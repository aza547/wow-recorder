...
const LinuxCaptureSettings = ({ recorderStatus, config, setConfig }: Props) => {
  ...
  React.useEffect(() => {
    ...
    setConfigValues({
      ...
      linuxGsrAudio: config.linuxGsrAudio,
    ...
    config.linuxGsrAudio,
    config.linuxGsrReplayStorage,
    config.linuxGsrLeadInSeconds,
  ]);

  const setNumber =
    (key: keyof ConfigurationSchema) => (e: React.ChangeEvent<HTMLInputElement>) => {
      const value = parseInt(e.target.value, 10);
      setConfig((prev) => ({ ...prev, [key]: Number.isFinite(value) ? value : 0 }));
    };

  const setString =
    (key: keyof ConfigurationSchema) => (value: string) => {
      setConfig((prev) => ({ ...prev, [key]: value }));
    };

  const setBool =
    (key: keyof ConfigurationSchema) => (checked: boolean) => {
  ...
  return (
    <div className="flex flex-col gap-y-4">
      <div className="text-sm text-foreground-lighter">
        Wayland capture uses the system portal. On first start, select the WoW window in the share dialog.
        <br />
        The replay buffer is only used to capture pre-roll (combat log events can be delayed). Full activities are recorded via a separate “regular recording” and are not limited by the buffer length.
      </div>
      ...
      <div className="flex flex-row flex-wrap gap-x-8 gap-y-4">
        ...
        <div className="flex flex-col w-[520px]">
          <Label htmlFor="linuxGsrAudio">Audio (GSR -a)</Label>
          <Input
            id="linuxGsrAudio"
            type="text"
            value={config.linuxGsrAudio}
            onChange={(e) => setString('linuxGsrAudio')(e.target.value)}
