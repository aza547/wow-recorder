...
const wowProcessStarted = (process: IWoWProcessResult) => {
  ...
  console.log(
    `[Logutils] Detected ${process.exe} (${process.flavour}) running`
  );
...
const wowProcessStopped = () => {
  ...
  console.log(
    `[Logutils] Detected ${wowProcessRunning.exe} (${wowProcessRunning.flavour}) not running`
  );
  wowProcessRunning = null;
 ...
 */
const runRetailRecordingTest = (endTest = true) => {
 ...
 * in the GUI. Uses some sample log lines from 2v2.txt.
 */
const runClassicRecordingTest = (endTest = true) => {
  console.log('[Logutils] User pressed the test button!');
