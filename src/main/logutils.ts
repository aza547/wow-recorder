 ...
 */
const runRecordingTest = (handler: LogHandler, endTest: boolean = true) => {
    console.log("[Logutils] User pressed the test button!");

    const parser = handler.combatLogParser;
    ...
    sendTestCombatLogLine(parser, testLines[0]);
    sendTestCombatLogLine(parser, testLines[1]);
    sendTestCombatLogLine(parser, testLines[2]);
    sendTestCombatLogLine(parser, testLines[3]);
    ...
    setTimeout(() => {
        sendTestCombatLogLine(parser, testArenaStopLine);
        testRunning = false;
    }, 10 * 1000);
}
