  ...
  return (
    ...
    >
      <Box component="span" sx={{ display: 'flex', alignItems: 'center' }}>
        <FormControl sx={{ my: 1 }}>
          <InputLabel id="obs-capture-mode-label" sx={style}>
            Capture Mode
          </InputLabel>
          <Select
            name="obsCaptureMode"
          ...
          >
            {Object.entries(obsCaptureModes).map((captureMode: any) => (
              <MenuItem
                key={`capture-mode-${captureMode[0]}`}
                value={captureMode[0]}
              >
                {captureMode[1]}
              </MenuItem>
          ...
          <FormControl sx={{ my: 1 }}>
            ...
            >
              {displayConfiguration.map((display: OurDisplayType) => (
                ...
                >
                  [{display.index + 1}] {display.size.width}x
                  {display.size.height} @ {display.displayFrequency} Hz (
                  {display.physicalPosition}){' '}
